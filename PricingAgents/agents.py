...
class PolicyAgent(BaseAgent):
    """Wraps a policy function (like Oracle) as an agent."""
...
class GreedyOLSAgent(BaseAgent):
    ...
    def act(self, ctx, t=None):
        ...
        b_hat = theta_hat[5]  # Correct: b is positive

        if b_hat <= 1e-6:
            return 0.5 * sum(self.price_bounds)
...
class ThompsonAgent(BaseAgent):
    def __init__(
        ...
        self.sigma_noise, self.price_bounds = sigma_noise, price_bounds
    ...
    def act(self, ctx, t=None):
        ...
        b_est = theta_sample[5]  # Correct: b is positive
